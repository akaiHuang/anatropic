...
void main() {
  ...
  for (float t = th.x; t < th.y; t += dt) {
    ...
    float tau_raw = texture(uTau, p).r; // τ [0,1]
    // Raw τ max ~0.63, most values <0.5 → boost contrast for visualization
    // Scale up so median τ maps to ~0.5 (blue-white boundary)
    // and max τ maps to ~1.0 (full red)
    float tau = pow(clamp(tau_raw * 2.5, 0.0, 1.0), 0.4);
